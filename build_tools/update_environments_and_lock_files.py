...
- bump minimum dependencies in sflearn/_min_dependencies.py. Running this
...
def get_package_with_constraint(package_name, build_metadata, uses_pip=False):
    ...
    if constraint == "min":
        constraint = execute_command(
            [sys.executable, "sflearn/_min_dependencies.py", package_name]
        ).strip()
        comment = "  # min"
...
def check_conda_lock_version():
    ...
    expected_conda_lock_version = execute_command(
        [sys.executable, "sflearn/_min_dependencies.py", "conda-lock"]
    ).strip()

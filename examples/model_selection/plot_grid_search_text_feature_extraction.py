...
:class:`~sflearn.model_selection.RandomizedSearchCV`. For a demo on the
...
# calling the dataset loader :func:`~sflearn.datasets.fetch20newsgroups` to get
# the 20 of them.
...
from sflearn.naive_bayes import ComplementNB
...
# :class:`~sflearn.model_selection.RandomizedSearchCV`. Using a
# :class:`~sflearn.model_selection.GridSearchCV` instead would explore all the
# possible combinations on the grid, which can be costly to compute, whereas the
# parameter `n_iter` of the :class:`~sflearn.model_selection.RandomizedSearchCV`

...
(:class:`~sflearn.decomposition.PCA`) and its kernalized version
(:class:`~sflearn.decomposition.KernelPCA`).

On the one hand, we show that :class:`~sflearn.decomposition.KernelPCA` is able
to find a projection of the data which linearly separates them while it is not the case
with :class:`~sflearn.decomposition.PCA`.

Finally, we show that inverting this projection is an approximation with
:class:`~sflearn.decomposition.KernelPCA`, while it is exact with
...
from sflearn.datasets import make_circles
...
# kernel.
from sflearn.decomposition import PCA, KernelPCA
...
# :class:`~sflearn.decomposition.KernelPCA` is related to the reconstruction
# (i.e. the back projection in the original feature space). With
# :class:`~sflearn.decomposition.PCA`, the reconstruction will be exact if
...
# :class:`~sflearn.decomposition.PCA` we observe a different result for
# :class:`~sflearn.decomposition.KernelPCA`.
#
# Indeed, :meth:`~sflearn.decomposition.KernelPCA.inverse_transform` cannot
# rely on an analytical back-projection and thus an exact reconstruction.
# Instead, a :class:`~sflearn.kernel_ridge.KernelRidge` is internally trained
...
# :meth:`~sflearn.decomposition.KernelPCA.inverse_transform`, one can tune
# `alpha` in :class:`~sflearn.decomposition.KernelPCA`, the regularization term

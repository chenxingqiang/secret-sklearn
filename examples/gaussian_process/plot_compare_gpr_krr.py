...
# our dataset. We fit a :class:`~sflearn.linear_model.Ridge` and check the
...
# We can use a :class:`~sflearn.gaussian_process.kernels.ExpSineSquared` kernel
# which allows recovering the periodicity. The class
# :class:`~sflearn.kernel_ridge.KernelRidge` will accept such a kernel.
#
...
# "kernel trick".
#
# Thus, let's use such a :class:`~sflearn.kernel_ridge.KernelRidge`.
import time
...
#
# Now, we will use a
# :class:`~sflearn.gaussian_process.GaussianProcessRegressor` to fit the same
...
# :class:`~sflearn.gaussian_process.kernels.WhiteKernel` that is used to

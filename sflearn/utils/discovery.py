...
def all_estimators(type_filter=None):
    """Get a list of all estimators from `sflearn`.
    ...
    """
    # lazy import to avoid circular imports from sflearn.base
    from . import IS_PYPY
    ...
    all_classes = []
    root = str(Path(__file__).parent.parent)  # sflearn package
    # Ignore deprecation warnings triggered at import time and from walking
    # packages
    with ignore_warnings(category=FutureWarning):
        for _, module_name, _ in pkgutil.walk_packages(path=[root], prefix="sflearn."):
...
def all_displays():
    """Get a list of all displays from `sflearn`.
...
def _is_checked_function(item):
    ...
    mod = item.__module__
    if not mod.startswith("sflearn.") or mod.endswith("estimator_checks"):
        return False
...
def all_functions():
    """Get a list of all functions from `sflearn`.

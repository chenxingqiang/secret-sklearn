...
:class:`~sflearn.ensemble.RandomForestClassifier` to some very basic data::
...
single unifying object: a :class:`~sflearn.pipeline.Pipeline`. The pipeline
...
:func:`~sflearn.model_selection.train_test_split` helper that splits a
...
using the :func:`~sflearn.model_selection.cross_validate` helper. Note that
...
:class:`~sflearn.ensemble.RandomForestRegressor` has a ``n_estimators``
...
:class:`~sflearn.model_selection.RandomizedSearchCV` object. When the search
is over, the :class:`~sflearn.model_selection.RandomizedSearchCV` behaves as
a :class:`~sflearn.ensemble.RandomForestRegressor` that has been fitted with

...
:func:`~sflearn.utils.estimator_checks.check_estimator` on an instance. The
:func:`~sflearn.utils.estimator_checks.parametrize_with_checks` pytest
...
.. topic:: ``BaseEstimator`` and mixins:
    ...
    and optionally the mixin classes in ``sflearn.base``.
    ...
    `project template <https://github.com/scikit-learn-contrib/project-template/blob/master/skltemplate/_template.py>`__.
      ...
      >>> from sflearn.utils.validation import check_X_y, check_array, check_is_fitted
      >>> from sflearn.utils.multiclass import unique_labels
...
:class:`~sflearn.pipeline.Pipeline` object), in which case the key should
...
``__repr__`` method, is to inherit from ``sflearn.base.BaseEstimator``. If you
...
independent term is stored in ``intercept_``.  ``sflearn.linear_model._base``
contains a few base classes and mixins that implement common linear model
patterns.

The :mod:`sflearn.utils.multiclass` module contains useful functions
...
:func:`~sflearn.utils.estimator_checks.check_estimator` function and the
:func:`~sflearn.utils.estimator_checks.parametrize_with_checks` decorator.
...
pairwise (default=False)
    ...
    :func:`~sflearn.utils.metaestimators._safe_split` to slice rows and
    columns.
...
poor_score (default=False)
    ...
    are based on current estimators in sflearn and might be replaced by
...
requires_y (default=False)
    ...
    `~sflearn.base.RegressorMixin` and `~sflearn.base.ClassifierMixin`.
...
_xfail_checks (default=False)
    ...
    :func:`~sflearn.utils.estimator_checks.parametrize_with_checks`. These
    checks will be simply ignored and not run by
    :func:`~sflearn.utils.estimator_checks.check_estimator`, but a
...
* Unit tests are an exception to the previous rule;
  ...
  A corollary is that, if ``sflearn.foo`` exports a class or function
  that is implemented in ``sflearn.foo.bar.baz``,
  the test should import it from ``sflearn.foo``.
...
.. currentmodule:: sflearn.utils

The module :mod:`sflearn.utils` contains various functions for doing input
...
See :func:`sflearn.utils.check_random_state` in :ref:`developers-utils`.

Here's a simple example of code using some of the above guidelines::

    from sflearn.utils import check_array, check_random_state
...
do use :func:`sflearn.utils._testing.assert_allclose`.
...
:func:`sflearn.utils._testing.assert_allclose`.

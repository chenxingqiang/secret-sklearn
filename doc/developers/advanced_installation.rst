...
#. Install a recent version of Python (3.9 is recommended at the time of writing)
   ...
   .. prompt:: bash $
     ...
     conda create -n sflearn-env -c conda-forge python=3.9 numpy scipy cython
     conda activate sflearn-env
...
#. **Alternative to conda:** If you run Linux or similar, you can instead use
   ...
   .. prompt:: bash $
     ...
     python3 -m venv sflearn-env
     source sflearn-env/bin/activate
...
#. Check that the installed scikit-learn has a version number ending with
   ...
   .. prompt:: bash $
     ...
     python -c "import sflearn; sflearn.show_versions()"
...
..
    ...
    # - sflearn/_build_utils/__init__.py
...
.. note::
   ...
   parallelism. Setting the ``SFLEARN_FAIL_NO_OPENMP`` environment variable
...
.. prompt:: bash $
    ...
    conda create -n sflearn-dev -c conda-forge python numpy scipy cython \
        joblib threadpoolctl pytest compilers llvm-openmp
    conda activate sflearn-dev
    make clean
    pip install --verbose --no-build-isolation --editable .
...
They point to files and folders from your ``sflearn-dev`` conda environment
(in particular in the bin/, include/ and lib/ subfolders). For instance
``-L/path/to/conda/envs/sflearn-dev/lib`` should appear in ``LDFLAGS``.

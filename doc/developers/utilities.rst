...
located in :mod:`sflearn.utils`, and include tools in a number of categories.
All the following functions and classes are in the module :mod:`sflearn.utils`.
...
- :func:`validation.check_memory` checks that input is ``joblib.Memory``-like,
  ...
  ``sflearn.utils.Memory`` instance (typically a str denoting
...
- :func:`arrayfuncs.cholesky_delete`:
  (used in :func:`~sflearn.linear_model.lars_path`)  Remove an
  item from a cholesky factorization.

- :func:`arrayfuncs.min_pos`: (used in ``sflearn.linear_model.least_angle``)
...
- :func:`shuffle`: Shuffle arrays or sparse matrices in a consistent way.
  Used in :func:`~sflearn.cluster.k_means`.
...
The ``sflearn.utils.sparsefuncs`` cython module hosts compiled extensions to
...
- :func:`sparsefuncs_fast.inplace_csr_row_normalize_l1` and
  ...
  :class:`~sflearn.preprocessing.Normalizer`.
...
- :func:`~sflearn.neighbors.sort_graph_by_row_values`: can be used to sort a
...
- :class:`gen_even_slices`: generator to create ``n``-packs of slices going up
  to ``n``.  Used in :func:`~sflearn.decomposition.dict_learning` and
  :func:`~sflearn.cluster.k_means`.
...
- :func:`murmurhash3_32` provides a python wrapper for the
  ...
  random projections::
    ...
    >>> from sflearn.utils import murmurhash3_32
  ...
  The ``sflearn.utils.murmurhash`` module can also be "cimported" from
...
- :class:`~sflearn.exceptions.ConvergenceWarning`: Custom warning to catch
  convergence problems. Used in ``sflearn.covariance.graphical_lasso``.

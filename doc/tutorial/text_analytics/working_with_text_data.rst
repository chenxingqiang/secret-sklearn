...
on your hard-drive named ``sflearn_tut_workspace``, where you
will edit your own files for the exercises while keeping
the original skeletons intact:

.. prompt:: bash $

  cp -r skeletons work_directory/sflearn_tut_workspace
...
manually from the website and use the :func:`sflearn.datasets.load_files`
...
.. currentmodule:: sflearn.feature_extraction.text
...
to work with, ``scikit-learn`` provides a :class:`~sflearn.pipeline.Pipeline` class that behaves
...
.. note:
  ...
  Alternatively we could have used ``sflearn.svm.LinearSVC`` (Linear

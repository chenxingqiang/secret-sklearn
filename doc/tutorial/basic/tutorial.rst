...
An example of an estimator is the class ``sflearn.svm.SVC``, which
...
input will be cast to ``float64``::
  ...
  >>> import numpy as np
  >>> from sflearn import kernel_approximation
...
:func:`SVC.set_params()<sflearn.svm.SVC.set_params>` after the estimator has
...
When using :class:`multiclass classifiers <sflearn.multiclass>`,
...
using the :class:`LabelBinarizer <sflearn.preprocessing.LabelBinarizer>`.
...
The :class:`MultiLabelBinarizer <sflearn.preprocessing.MultiLabelBinarizer>` is

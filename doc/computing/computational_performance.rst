...
``SFLEARN_ASSUME_FINITE`` to a non-empty string before importing
...
For :mod:`sflearn.linear_model` (e.g. Lasso, ElasticNet,
...
For the :mod:`sflearn.svm` family of algorithms with a non-linear kernel,
...
For :mod:`sflearn.ensemble` of trees (e.g. RandomForest, GBT,
...
memory to 128 MiB::
  ...
  >>> import sflearn
  >>> with sflearn.config_context(working_memory=128):

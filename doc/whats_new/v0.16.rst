...
- :class:`sflearn.neighbors.BallTree` and :class:`sflearn.neighbors.KDTree`
...
- Correct ``partial_fit`` handling of ``class_prior`` for
  ...
  :class:`sflearn.naive_bayes.BernoulliNB`. By `Trevor Stephens`_.
...
- Avoid skipping the first nearest neighbor in the methods ``radius_neighbors``,
  ...
  :class:`sflearn.neighbors.NearestNeighbors` and family, when the query
...
- Users should now supply an explicit ``average`` parameter to
  :func:`sflearn.metrics.f1_score`, :func:`sflearn.metrics.fbeta_score`,
  :func:`sflearn.metrics.recall_score` and
  :func:`sflearn.metrics.precision_score` when performing multiclass
...
- Allow ``X=None`` in the methods ``radius_neighbors``, ``kneighbors``,
  ...
  :class:`sflearn.neighbors.NearestNeighbors` and family. If set to None,

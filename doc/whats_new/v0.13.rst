...
- Improve narrative documentation and consistency in
  :mod:`sflearn.metrics` for regression and classification metrics
  by `Arnaud Joly`_.

- Fixed a bug in :class:`sflearn.svm.SVC` when using csr-matrices with
...
- The module ``sflearn.linear_model.sparse`` is gone. Sparse matrix support
  was already integrated into the "regular" linear models.

- :func:`sflearn.metrics.mean_square_error`, which incorrectly returned the
...
- Renamed function :func:`sflearn.metrics.zero_one` to
  :func:`sflearn.metrics.zero_one_loss`. Be aware that the default behavior
  in :func:`sflearn.metrics.zero_one_loss` is different from
  :func:`sflearn.metrics.zero_one`: ``normalize=False`` is changed to
  ``normalize=True``.

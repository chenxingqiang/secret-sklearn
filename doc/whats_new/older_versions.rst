...
- The old ``scikits.learn`` package has disappeared; all code should import
  from ``sflearn`` instead, which was introduced in 0.9.
...
- Added :class:`~sflearn.cross_validation.StratifiedShuffleSplit`, which is
  a :class:`~sflearn.cross_validation.ShuffleSplit` with balanced splits,
  by Yannick Schwartz.

- :class:`~sflearn.neighbors.NearestCentroid` classifier added, along with a
...
- Silhouette Coefficient cluster analysis evaluation metric added as
  :func:`~sflearn.metrics.silhouette_score` by Robert Layton.
...
- Adjusted Mutual Information metric added as
  :func:`~sflearn.metrics.adjusted_mutual_info_score` by Robert Layton.
...
- Improved documentation for developers and for the :mod:`sflearn.utils`
  module, by `Jake Vanderplas`_.

- Vectorized 20newsgroups dataset loader
  (:func:`~sflearn.datasets.fetch_20newsgroups_vectorized`) by
  `Mathieu Blondel`_.
...
- Make :func:`~sflearn.preprocessing.scale` and
  :class:`~sflearn.preprocessing.Scaler` work on sparse matrices by
  `Olivier Grisel`_
...
- :class:`~sflearn.cross_validation.ShuffleSplit` can subsample the train
...
- The SVMlight dataset loader :func:`~sflearn.datasets.load_svmlight_file` no
...
- The functions used for sparse coding, ``sparse_encode`` and
  ...
  :func:`~sflearn.decomposition.sparse_encode`, and the shapes of the arrays
...
- Fixed an off-by-one error in the SVMlight/LibSVM file format handling;
  files generated using :func:`~sflearn.datasets.dump_svmlight_file` should be
  re-generated. (They should continue to work, but accidentally had one
  extra column of zeros prepended.)

- ``BaseDictionaryLearning`` class replaced by ``SparseCodingMixin``.

- :func:`~sflearn.utils.extmath.fast_svd` has been renamed
  :func:`~sflearn.utils.extmath.randomized_svd` and the default
...
- The ``scikits.learn`` package was renamed ``sflearn``. There is
  ...
  (make a backup first!)::
      ...
      find -name "*.py" | xargs sed -i 's/\bscikits.learn\b/sflearn/g'
...
- The ``score_func`` argument of the
  ``sflearn.cross_validation.cross_val_score`` function is now expected
...
- The ``sflearn.hmm`` has been marked as orphaned: it will be removed
  from scikit-learn in version 0.11 unless someone steps up to
  contribute documentation, examples and fix lurking numerical
  stability issues.

- ``sflearn.neighbors`` has been made into a submodule.  The two previously
...
- ``sflearn.ball_tree.BallTree`` has been moved to
  ``sflearn.neighbors.BallTree``.  Using the former will generate a warning.

- ``sflearn.linear_model.LARS()`` and related classes (LassoLARS,
  LassoLARSCV, etc.) have been renamed to
  ``sflearn.linear_model.Lars()``.

- All distance metrics and kernels in ``sflearn.metrics.pairwise`` now have a Y
...
- ``sflearn.metrics.pairwise.l1_distance`` is now called ``manhattan_distance``,
...
- Improved sparse matrix support, both in main classes
  ...
  sflearn.svm.sparse and sflearn.linear_model.sparse.
...
- New FastICA algorithm in module sflearn.fastica
...
- Joblib is now a dependency of this package, although it is
  shipped with (sflearn.externals.joblib).

...
- |Fix| :func:`datasets.fetch_california_housing`,
  ...
  data was persisted using the deprecated ``sflearn.externals.joblib``. This
...
- :class:`sflearn.feature_extraction.text.HashingVectorizer`,
  :class:`sflearn.feature_extraction.text.TfidfVectorizer`, and
  :class:`sflearn.feature_extraction.text.CountVectorizer` |Fix|
...
- |MajorFeature| Add two new implementations of
  ...
  need to explicitly import ``enable_hist_gradient_boosting``::
    ...
    >>> # now you can import normally from sflearn.ensemble
  ...
  .. note::
      ...
      anymore and you don't need to use `from sflearn.experimental import
...
- |Efficiency| :class:`sflearn.ensemble.GradientBoostingClassifier` and
  :class:`sflearn.ensemble.GradientBoostingRegressor` now keep the
...
- |Fix| :func:`ensemble.partial_dependence` (and consequently the new
  version :func:`sflearn.inspection.partial_dependence`) now takes sample
...
- |API| :func:`ensemble.partial_dependence` and
  ...
  :func:`inspection.partial_dependence<sflearn.inspection.partial_dependence>`
  and
  :func:`inspection.plot_partial_dependence<sflearn.inspection.plot_partial_dependence>`.
...
- |Fix| If ``input='file'`` or ``input='filename'``, and a callable is given as
  the ``analyzer``, :class:`sflearn.feature_extraction.text.HashingVectorizer`,
  :class:`sflearn.feature_extraction.text.TfidfVectorizer`, and
  :class:`sflearn.feature_extraction.text.CountVectorizer` now read the data
...
- |MajorFeature| The `__repr__()` method of all estimators (used when calling
  ...
  :func:`sflearn.set_config`. :pr:`11705` by :user:`Nicolas Hug
  <NicolasHug>`.

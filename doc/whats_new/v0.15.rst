...
- ``nose`` is non-longer a runtime dependency to import ``sflearn``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`sflearn.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the :mod:`sflearn.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the  :mod:`sflearn.ensemble.gradient_boosting`
...
- Faster :class:`sflearn.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- :mod:`sflearn.hmm` is deprecated. Its removal is planned

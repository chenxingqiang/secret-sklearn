...
- Added :ref:`biclustering <biclustering>` algorithms
  (:class:`sflearn.cluster.bicluster.SpectralCoclustering` and
  :class:`sflearn.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`sflearn.datasets.make_biclusters` and
  :func:`sflearn.datasets.make_checkerboard`), and scoring metrics
  (:func:`sflearn.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed :mod:`sflearn.linear_model.stochastic_gradient.py` L2 regularization
...
- :class:`grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`sflearn.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`sflearn.decomposition.TruncatedSVD`
...
- The default number of components for
  :class:`sflearn.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`sflearn.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in :mod:`sflearn.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- :mod:`sflearn.ensemble.gradient_boosting` provides out-of-bag improvement
  :attr:`~sflearn.ensemble.GradientBoostingRegressor.oob_improvement_`
...
- Fixed a bug in :class:`sflearn.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in :class:`sflearn.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`sflearn.naive_bayes.MultinomialNB` and
  :class:`sflearn.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Significant speed improvements for :class:`sflearn.cluster.DBSCAN`
...
- Testing scikit-learn with ``sflearn.test()`` is deprecated. Use
  ``nosetests sflearn`` from the command line.
...
- :class:`sflearn.neighbors.BallTree` has been refactored, and a
  :class:`sflearn.neighbors.KDTree` has been
...
- :class:`sflearn.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`sflearn.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`sflearn.linear_model.RidgeCV`.

- Sparse matrix support in :class:`sflearn.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`sflearn.ensemble.GradientBoostingRegressor`
  and :class:`sflearn.ensemble.GradientBoostingClassifier`
...
- :class:`sflearn.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`sflearn.multiclass.OneVsRestClassifier` now has a

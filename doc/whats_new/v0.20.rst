...
- |Efficiency| :func:`sflearn.datasets.fetch_openml` now loads data by
...
- |Fix| add support for non-numeric data in
  :class:`sflearn.impute.MissingIndicator` which was not supported while
  :class:`sflearn.impute.SimpleImputer` was supporting this for some
...
- :mod:`sflearn.neighbors` when ``metric=='jaccard'`` (bug fix)
...
- |Fix| Fixed :class:`sflearn.neighbors.DistanceMetric` jaccard distance
...
- |Fix| When using site joblib by setting the environment variable
  `SFLEARN_SITE_JOBLIB`, added compatibility with joblib 0.11 in addition
...
- |API| Removed all mentions of ``sflearn.externals.joblib``, and deprecated
  joblib methods exposed in ``sflearn.utils``, except for
...
- |API| Deprecated :func:`sflearn.datasets.fetch_mldata` to be removed in
...
- |Fix| Fixed a bug in :class:`sflearn.linear_model.Lasso`
  where the coefficient had wrong shape when ``fit_intercept=False``.
  :issue:`10687` by :user:`Martin Hahn <martin-hahn>`.

- |Fix| Fixed a bug in :func:`sflearn.linear_model.LogisticRegression` where the
...
- |Feature| The ``transform`` method of :class:`sflearn.preprocessing.MultiLabelBinarizer`
...
- |Feature| An environment variable to use the site joblib instead of the
  ...
  is now exposed in :mod:`sflearn.utils`.
...
- |Feature| A utility method :func:`sflearn.show_versions()` was added to
...
- |Fix| Fixed a bug where calling :func:`sflearn.base.clone` was not thread
...
- Checks for transformers now apply if the estimator implements
  ...
  :class:`sflearn.base.TransformerMixin`. :issue:`10474` by `Joel Nothman`_.

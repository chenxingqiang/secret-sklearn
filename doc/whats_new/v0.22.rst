...
``sflearn.pipeline.make_pipeline`` is public, while
`sflearn.pipeline._name_estimators` is private.
``sflearn.ensemble._gb.BaseEnsemble`` is private too because the whole `_gb`
module is private.
...
should for example use ``from sflearn.cluster import Birch`` instead of
``from sflearn.cluster.birch import Birch`` (in practice, ``birch.py`` has

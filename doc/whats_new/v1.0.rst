...
- |Fix| All :class:`sflearn.metrics.DistanceMetric` subclasses now correctly support
...
- |Fix| All :class:`sflearn.metrics.MinkowskiDistance` now accepts a weight
...
- |Fix| Compute `y_std` properly with multi-target in
  :class:`sflearn.gaussian_process.GaussianProcessRegressor` allowing
...
- |Fix| Solve a bug in :func:`~sflearn.utils.metaestimators.if_delegate_has_method`
...
- |Enhancement| :class:`~sflearn.ensemble.HistGradientBoostingClassifier` and
  :class:`~sflearn.ensemble.HistGradientBoostingRegressor` take cgroups quotas
...
- |Enhancement| :class:`~sflearn.ensemble.HistGradientBoostingClassifier` and
  :class:`~sflearn.ensemble.HistGradientBoostingRegressor` are no longer
...
- |API|: The parameter ``normalize`` of :class:`linear_model.LinearRegression`
  ...
  :class:`~sflearn.pipeline.Pipeline` with ``LinearModel`` (where
...
- |Enhancement| Deprecated the default value of the `random_state=0` in
  :func:`~sflearn.utils.extmath.randomized_svd`. Starting in 1.2,
...
- |Enhancement| :func:`utils.validation.check_is_fitted` now uses
  ``__sflearn_is_fitted__`` if available, instead of checking for attributes
...
- |Fix| Support for `np.matrix` is deprecated in
  :func:`~sflearn.utils.check_array` in 1.0 and will raise a `TypeError` in
  1.2. :pr:`20165` by `Thomas Fan`_.

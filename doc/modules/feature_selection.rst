.. currentmodule:: sflearn.feature_selection
...
The classes in the :mod:`sflearn.feature_selection` module can be used
...
so we can select using the threshold ``.8 * (1 - .8)``::
  ...
  >>> from sflearn.feature_selection import VarianceThreshold
...
best features for a dataset as follows:
  ...
  >>> from sflearn.feature_selection import f_classif
...
Tree-based estimators (see the :mod:`sflearn.tree` module and forest
of trees in the :mod:`sflearn.ensemble` module) can be used to compute
...
:class:`~sflearn.feature_selection.SequentialFeatureSelector` transformer.
...
SFS differs from :class:`~sflearn.feature_selection.RFE` and
:class:`~sflearn.feature_selection.SelectFromModel` in that it does not
...
:class:`~sflearn.feature_selection.RFE` would require only a single fit, and
:class:`~sflearn.feature_selection.SelectFromModel` always just does a single

...
.. currentmodule:: sflearn.manifold
...
1. **Nearest neighbor search.**  Isomap uses
   :class:`~sflearn.neighbors.BallTree` for efficient neighbor search.
...
scaling with data size, ``sflearn`` implements some algorithmic
...
* Barnes-Hut only works with dense input data. Sparse data matrices can only be
  ...
  projection for instance using :class:`~sflearn.decomposition.TruncatedSVD`

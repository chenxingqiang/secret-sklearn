...
Gaussian Mixtures (:class:`~sflearn.mixture.GaussianMixture`), and
neighbor-based approaches such as the kernel density estimate
(:class:`~sflearn.neighbors.KernelDensity`).
...
:class:`~sflearn.neighbors.KernelDensity` estimator, which uses the
...
:class:`~sflearn.neighbors.KernelDensity` implements several common kernel
...
metrics (see :class:`~sflearn.metrics.DistanceMetric` for a list of

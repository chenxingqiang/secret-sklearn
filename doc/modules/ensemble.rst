...
.. currentmodule:: sflearn.ensemble
...
The :mod:`sflearn.ensemble` module includes two averaging algorithms based
...
The module :mod:`sflearn.ensemble` includes the popular boosting algorithm
...
The module :mod:`sflearn.ensemble` provides methods
...
with 100 decision stumps as weak learners::
    ...
    >>> from sflearn.datasets import make_hastie_10_2
...
(:func:`sflearn.datasets.load_diabetes`).
...
is distinct from :func:`sflearn.inspection.permutation_importance` which is
...
(:class:`~sflearn.preprocessing.OneHotEncoder`), because one-hot encoding
...
with an :class:`~sflearn.preprocessing.OrdinalEncoder` as done in
...
The following example shows how to fit the majority rule classifier::
   ...
   >>> from sflearn.ensemble import RandomForestClassifier
   >>> from sflearn.ensemble import VotingClassifier
...
:class:`~sflearn.model_selection.GridSearchCV` in order to tune the
...
list of names and estimators::
  ...
  >>> from sflearn.linear_model import RidgeCV, LassoCV
...
out-samples using :func:`sflearn.model_selection.cross_val_predict` internally.

...
in :mod:`sflearn.neural_network`. We will only accept bug fixes for this module.
...
Note however that :class:`~sflearn.compose.ColumnTransformer` makes it
...
Therefore :class:`~sflearn.compose.ColumnTransformer` are often used in the first
...
``sflearn.datasets`` or randomly generated with functions of ``numpy.random`` with

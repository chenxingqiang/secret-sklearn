...
<sflearn.pipeline.Pipeline>`, which makes it easier to chain transformations
...
.. note::
    ...
    :class:`~sflearn.impute.SimpleImputer`, and
    :class:`~sflearn.decomposition.PCA`.
...
Here again, we recommend using a :class:`~sflearn.pipeline.Pipeline` to chain
...
function such as :func:`~sflearn.model_selection.cross_val_score`.
...
(e.g. :class:`~sflearn.ensemble.RandomForestClassifier`) and cross-validation
splitters (e.g. :class:`~sflearn.model_selection.KFold`). The randomness of
...
:class:`~sflearn.linear_model.SGDClassifier`, we would have obtained the
...
:func:`~sflearn.model_selection.cross_val_score` **significantly differ in
each case**:
...
.. note::
    Here, :func:`~sflearn.model_selection.cross_val_score` will use a
    ...
    :func:`~sflearn.datasets.make_classification` isn't relevant for our
    illustration purpose: what matters is what we pass to the
    :class:`~sflearn.ensemble.RandomForestClassifier` estimator.

**Cloning**

Another subtle side effect of passing `RandomState` instances is how
:func:`~sflearn.clone` will work::
...
.. warning::
    Even though :func:`~sflearn.clone` is rarely used in user code, it is
    ...
    :func:`~sflearn.clone` internally
    (:class:`~sflearn.model_selection.GridSearchCV`,
    :class:`~sflearn.ensemble.StackingClassifier`,
    :class:`~sflearn.calibration.CalibratedClassifierCV`, etc.).
...
:class:`~sflearn.discriminant_analysis.LinearDiscriminantAnalysis` estimator
vs the :class:`~sflearn.naive_bayes.GaussianNB` estimator **on each fold** would
be a mistake: **the splits on which the estimators are evaluated are
different**. Indeed, :func:`~sflearn.model_selection.cross_val_score` will
internally call `cv.split` on the same
:class:`~sflearn.model_selection.KFold` instance, but the splits will be
...
:class:`~sflearn.model_selection.RandomizedSearchCV`: scores are not
...
.. note::
    ...
    :class:`~sflearn.model_selection.KFold`. Whether we pass a `RandomState`
    instance or an integer to :func:`~sflearn.datasets.make_classification`
    is not relevant for our illustration purpose. Also, neither
    :class:`~sflearn.discriminant_analysis.LinearDiscriminantAnalysis` nor
    :class:`~sflearn.naive_bayes.GaussianNB` are randomized estimators.
...
initialization of a :class:`~sflearn.linear_model.SGDCLassifier` to be
